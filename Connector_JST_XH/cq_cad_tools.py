...
def FuseObjs_wColors(App, Gui,
    ...
    bla = []
    for part in parts:
        bla.append(App.ActiveDocument.getObject(part.Name))

    #App.activeDocument().Fusion.Shapes = [App.ActiveDocument.getObject(parts[0].Name), App.ActiveDocument.getObject(parts[1].Name), App.ActiveDocument.getObject(parts[2].Name)]
    App.activeDocument().Fusion.Shapes = bla
    Gui.ActiveDocument.Fusion.ShapeColor=Gui.ActiveDocument.getObject(parts[0].Name).ShapeColor
    Gui.ActiveDocument.Fusion.DisplayMode=Gui.ActiveDocument.getObject(parts[0].Name).DisplayMode
    App.ActiveDocument.recompute()

    App.ActiveDocument.addObject('Part::Feature',"Fusion").Shape=App.ActiveDocument.Fusion.Shape
    fusion = App.ActiveDocument.ActiveObject
...
#
###################################################################
def exportSTEP(doc,modelName, dir, objs):
...
def exportVRML(doc,modelName,scale,dir):
    ...
    vmrl = []
    for o in objs:
        FreeCAD.ActiveDocument.addObject('Part::Feature','scaled__'+o.Label).Shape=o.Shape
        FreeCAD.ActiveDocument.ActiveObject.Label='scaled__'+o.Label
        FreeCADGui.ActiveDocument.ActiveObject.ShapeColor=FreeCADGui.getDocument(doc.Name).getObject(o.Name).ShapeColor
        FreeCADGui.ActiveDocument.ActiveObject.LineColor=FreeCADGui.getDocument(doc.Name).getObject(o.Name).LineColor
        FreeCADGui.ActiveDocument.ActiveObject.PointColor=FreeCADGui.getDocument(doc.Name).getObject(o.Name).PointColor
        FreeCADGui.ActiveDocument.ActiveObject.DiffuseColor=FreeCADGui.getDocument(doc.Name).getObject(o.Name).DiffuseColor
        FreeCAD.ActiveDocument.recompute()
        newObj=FreeCAD.getDocument(doc.Name).getObject('scaled__'+o.Label)
    ...
    FreeCADGui.export(vmrl,VrmlFileName)
    FreeCAD.activeDocument().recompute()
    del vmrl
    return 0
